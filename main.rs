...
async fn main() {
    ...
    if seed_addrs.is_empty() {
        error!("No valid seed addresses found. Exiting.");
        return;
    }
    ...
    let nodes_snapshot = Arc::new(Mutex::new(std::collections::HashMap::new()));
    ...
    // Wait for a single successful connection (with warning instead of panic)
    info!("Waiting for initial connection...");
    let mut connected = false;
    for _ in 0..30 {
        if crawler.node().num_connected() >= 1 {
            connected = true;
            break;
        }
    ...
    if !connected {
        warn!("Could not establish initial connection. Continuing anyway...");
    }

    // Wait for seed nodes to respond (with warning instead of panic)
    info!("Waiting for peer addresses...");
    let start = Instant::now();
    while start.elapsed() < Duration::from_millis(SEED_RESPONSE_TIMEOUT_MS) {
        if crawler.known_network.nodes().len() > seed_addrs.len() {
            info!("Received peer addresses!");
            break;
        }
    ...
    if crawler.known_network.nodes().len() <= seed_addrs.len() {
        warn!("No new addresses received from seeds. Network may be unreachable.");
    }
        ...
        loop {
            ...
            if crawler.known_network.num_connections() > 0 {
                ...
                *summary_snapshot.lock() = new_summary;
                ...
                // Actualizar snapshot de nodos para el RPC
