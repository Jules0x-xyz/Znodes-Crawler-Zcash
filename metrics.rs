...
pub fn new_network_summary(crawler: &Crawler, graph: &Graph<SocketAddr>) -> NetworkSummary {
    ...
    // Build adjacency indices manually since get_filtered_adjacency_indices doesn't exist
    let nodes_indices: Vec<Vec<usize>> = good_nodes
        .iter()
        .enumerate()
        .map(|(_, node_addr)| {
            let mut neighbors = Vec::new();
            for (idx, other_addr) in good_nodes.iter().enumerate() {
                if node_addr != other_addr && graph.contains(&Edge::new(*node_addr, *other_addr)) {
                    neighbors.push(idx);
                }
            }
            neighbors
